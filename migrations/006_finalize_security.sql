-- ============================================================
-- 006_finalize_security.sql
-- Resolves ALL Supabase Security Advisor Warnings & Informational issues
-- by enforcing exact search paths, explicit RLS policies, and strict grants.
-- ============================================================

-- ------------------------------------------------------------
-- 1. NUKE PREVIOUS FUNCTIONS TO PREVENT GHOSTING WARNINGS
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 2. REBUILD FUNCTIONS WITH SPECIFIED SEARCH_PATH
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 3. APPLY PRECISE EXECUTION GRANTS
-- ------------------------------------------------------------
-- Strip all
REVOKE ALL ON FUNCTION public.handle_new_user() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.is_staff() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.reserve_power_bank(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.release_expired_reservations() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.submit_rental_request(text, uuid, text, numeric, text, text, text, text, text, text, text, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_public_tracking_status(text) FROM PUBLIC;
...
-- We do NOT grant handle_new_user or reserve_power_bank to anon or authenticated. They are internal/trigger only.
...
-- ------------------------------------------------------------
-- 4. CLEAN AND REAPPLY RLS POLICIES
-- Resolves "RLS Enabled No Policy" and completely shields data.
-- ------------------------------------------------------------

-- A. Erase ghost policies completely just in case
...
-- B. Apply Final Explicit Policies
...
-- power_banks
-- Customers need to see inventory, but restricted to safe states
...
-- customers
-- Customers insert/read ONLY via RPC. NO direct table access.
...
-- rentals
-- Customers read ONLY via RPC. NO direct table access.
CREATE POLICY "Staff all rentals" ON public.rentals FOR ALL USING (public.is_staff());

-- payments
-- Customers insert ONLY via RPC. NO direct table access.
CREATE POLICY "Staff all payments" ON public.payments FOR ALL USING (public.is_staff());

-- activity_logs
-- Append-only for staff. Selectable for staff. Nobody updates/deletes.
